...
namespace Vueling.XXX.DB.Infrastructure.MapFactories.MapConceptualModelToDomain.AircraftRepository
{
    class AircraftDbObjectToAircraftEntityFactory : MapConceptualModelToDomainFactoryBase
    {
        ...
        internal override TOutput GetEntityFromDbObject<TInput, TOutput>(TInput dbobject)
        {
            ...
            var aircraft = dbobject as Vueling.XXX.DB.Infrastructure.Repositories.AircraftRepository.Aircraft;//if casting fail, return null, so we have to verify the result
            if (aircraft == null) { throw new InvalidCastException("Cast to type Vueling.XXX.Infrastructure.Repositories.AircraftRepository.Aircraft has fail."); }
