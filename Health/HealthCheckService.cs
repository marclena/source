...
{
    internal class HealthCheckService // : HealthCheckService
    {
            ...
            foreach (var healthCheck in healthChecks)
            {
                ...
                */
                ...
                using (_logger.BeginScope(new HealthCheckLogScope(healthCheck.GetType().Name)))
                {
                    ...
                    PropertyInfo myPropInfo = healthCheck.GetType().GetProperty("HealthCheckName");
                    string name = myPropInfo.GetValue(healthCheck) as string;

                   // entries[healthCheck.GetType().Name + i] = entry;
                    entries[name] = entry;
